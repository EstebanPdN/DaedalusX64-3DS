...
bad=[p for p in paths if p and p not in source_exceptions and (pathlib.Path(p).suffix.lower() in blocked or p.startswith(('.deps/','Artifacts/','Build/','Research/','Dumps/','dump-')))]
if bad:
